...
const Submission = {
    ...
    grade: async (submissionId, { score, creator_feedback }) => {
        const query = `
            UPDATE submissions
            SET score = $1, creator_feedback = $2, evaluation_date = CURRENT_TIMESTAMP
            WHERE id = $3
            RETURNING *;
        `;
        const params = [score, creator_feedback, submissionId];
        const { rows } = await db.query(query, params);
        return rows[0];
    },
    ...
    getRankingByCompetitionId: async (competitionId) => {
        const query = `
            ...
            ORDER BY s.score DESC, s.submission_date ASC; -- Ordena por maior nota, e depois por data de envio como desempate
        `;
        const { rows } = await db.query(query, [competitionId]);
        return rows;
    },
